...
本模块包含三个子模块，每个子模块负责特定的诊断维度：

- **[server_health_checks](server_health_checks.md)** — Dolt 服务器模式的健康检查，包括 TCP 连通性、版本验证、数据库存在性、schema 兼容性、连接池健康和残留数据库检测
- **[migration_validation](migration_validation.md)** — 迁移前后的数据完整性验证，包括 JSONL 完整性检查、Dolt 与 JSONL 数据比对、跨 rig 污染检测
- **[migration_detection](migration_detection.md)** — 待执行迁移任务的检测，识别需要运行的迁移命令并提供优先级排序
