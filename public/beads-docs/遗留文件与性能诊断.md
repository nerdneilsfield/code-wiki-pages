...
flowchart TB
    subgraph "遗留文件与性能诊断"
        A[artifacts.go<br/>遗留文件扫描] --> |输出| AR[ArtifactReport]
        B[perf_dolt.go<br/>Dolt性能诊断] --> |输出| DM[DoltPerfMetrics]
    end
    
    subgraph "Doctor 框架"
        DC[DoctorCheck<br/>诊断结果结构]
    end
    
    subgraph "依赖模块"
        DS[internal.doltserver<br/>服务器配置]
        CF[internal.configfile<br/>配置文件]
        SD[internal.storage.dolt<br/>后端检测]
    end
    
    subgraph "下游消费者"
        CLI[bd doctor 命令行]
        FIX[cmd.bd.doctor.fix<br/>修复逻辑]
    end
