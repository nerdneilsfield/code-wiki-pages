...
详细信息请参考 [agent_identity_and_discovery.md](agent_identity_and_discovery.md)。
...
详细信息请参考 [remote_agent_client.md](remote_agent_client.md)。
...
详细信息请参考 [task_lifecycle_management.md](task_lifecycle_management.md)。
...
详细信息请参考 [sse_event_streaming.md](sse_event_streaming.md)。
...
为避免在主文档中重复底层实现细节，A2A Protocol 的核心子模块已拆分为独立文档。建议按“发现 → 提交与控制 → 流式反馈 → 生命周期管理”的顺序阅读：

1. **agent_discovery_card**：`[agent_discovery_card.md](agent_discovery_card.md)`  
   重点说明 AgentCard 的协议定位、`/.well-known/agent.json` 暴露机制、默认技能与认证声明、`toJSON`/`handleRequest` 的实现语义，以及能力声明与真实执行能力的一致性风险。

2. **remote_agent_client**：`[remote_agent_client.md](remote_agent_client.md)`  
   重点说明 A2AClient 的请求封装模型（discover/submit/get/cancel/stream）、`_request` 的超时与响应体限制、SSE 解析边界，以及如何在编排层做重试与错误映射。

3. **sse_event_streaming**：`[sse_event_streaming.md](sse_event_streaming.md)`  
   重点说明 SSEStream 的缓冲-刷新机制、事件发送 API（`sendProgress`/`sendArtifact`/`sendStateChange`）、连接关闭路径、写入异常行为和运行时内存控制建议。

4. **task_lifecycle_management**：`[task_lifecycle_management.md](task_lifecycle_management.md)`  
   重点说明 TaskManager 的状态机约束、终态保护、输入大小与任务容量限制、事件模型（`task:created`/`task:stateChange`/`task:updated`）及分布式场景下的限制。

### 与其他模块的交叉阅读建议

- 若你在做服务端接入与路由暴露，可结合 `[API Server & Services.md](API Server & Services.md)`。
- 若你在做多智能体编排，可结合 `[Swarm Multi-Agent.md](Swarm Multi-Agent.md)`。
- 若你在做前后端实时观测，可结合 `[Dashboard Backend.md](Dashboard Backend.md)` 与 `[Dashboard UI Components.md](Dashboard UI Components.md)`。
- 若你在做 SDK 封装，可结合 `[Python SDK.md](Python SDK.md)` 与 `[TypeScript SDK.md](TypeScript SDK.md)`。

### 端到端职责边界总结

```mermaid
flowchart LR
    Discover[agent_discovery_card\n声明身份与能力] --> Invoke[remote_agent_client\n发起远程调用]
    Invoke --> Lifecycle[task_lifecycle_management\n驱动任务状态机]
    Lifecycle --> Streaming[sse_event_streaming\n实时推送状态与产物]
    Streaming --> Observe[Dashboard / SDK / 外部调用方]
```

该职责链体现了 A2A Protocol 的核心设计思想：**用最小协议原语解耦“能力发现、任务控制、状态演进、实时反馈”四类问题**。当需要认证、审计、策略治理、持久化与多租户隔离时，应由外围模块（Policy Engine、Audit、Dashboard Backend、State Management 等）补齐。