...
`document_options_and_callbacks` 模块是整个文档处理系统的基础设施层，它通过精心设计的选项系统和回调机制，解决了框架设计中的一个核心难题：如何在保持统一接口的同时，支持灵活的实现特定扩展。

通过学习这个模块，新加入团队的开发者应该理解：

1. **类型擦除模式**：如何使用 `any` 类型和包装函数来实现"统一接口 + 实现特定扩展"的平衡
2. **回调契约设计**：如何定义灵活的回调数据结构，支持简洁用法和完整用法并存
3. **组件协作模式**：Loader、Parser、Transformer 如何通过选项和回调系统协同工作

这个模块的设计理念体现了框架设计的精髓：提供足够的结构来确保一致性和可维护性，同时保留足够的灵活性来支持各种实现和使用场景。
